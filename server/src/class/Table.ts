...
class Table {
  ...
  public blackjackEvaluateAndGetRoundResults(): string {
    ...
    while (!this.onLastPlayer()) {
      ...
      // bust
      if (currPlayer.getAction() === "bust")
        currPlayer.setWinAmount(-currPlayer.getBet());
      // surrender
      else if (currPlayer.getAction() === "surrender")
        currPlayer.setWinAmount(-Math.floor(currPlayer.getBet() / 2));
      else {
        // game with house
        if (this.house.getAction() === "bust") {
          currPlayer.setWinAmount(
            currPlayer.blackjack()
              ? Math.floor(currPlayer.getBet() * 1.5)
              : currPlayer.getAction() === "double"
              ? currPlayer.getBet() * 2
              : currPlayer.getBet()
          ); // playerがwin
        } else {
          ...
          if (this.house.blackjackAndHasAce()) {
            ...
            if (!currPlayer.blackjackAndHasAce())
              currPlayer.setWinAmount(
                currPlayer.getAction() === "double"
                  ? -currPlayer.getBet() * 2
                  : -currPlayer.getBet()
  ...
  */
  public blackjackAssignPlayerHands(player: House | User | Ai): void {
    if (this.canPlayForBlackjack(player)) {
      if (player == this.house) player.addCard(this.deck.drawOne());
      ...
      if (this.gamePhase == "waitingForBets") {
        ...
        if (currPlayer != this.house) {
          ...
          if (this.actionsResolved(currPlayer)) {
            ...
            this.evaluateMove(currPlayer, userData);
            if (
              currPlayer.getHand().length == 2 &&
              currPlayer.getAction() === "double"
            )
              currPlayer.getHand().push(this.deck.drawOne());
            if (!this.actionsResolved(currPlayer))
              currPlayer.getHand().push(this.deck.drawOne());
            this.turnCounter++;
          }
  ...
  public actionsResolved(player: User | House | Ai | null = null) {
    ...
    if (player === null) {
      for (let i = 0; i < this.playerNumber - 1; i++) {
        if (!(this.players[i].getAction() in Table.ACTIONMAP))
          hasAction = false;
      }
    } else {
  ...
  */
  ...
  public switchGamePhase(gamePhase: { [key: string]: string }) {
    ...
    if (this.gamePhase === "roundOver" && this.user.getStatus() === "broke")
  ...
  */
  ...
  // setter
  public setGameType(gameType: TGameType): void {
    this.gameType = gameType;
  }

  public setBetDenominations(betDenominations: number[]): void {
    this.betDenominations = betDenominations;
  }
  ...
  public setPlayers(players: (House | Ai | User)[]): void {
